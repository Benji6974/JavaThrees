...
public class Grille {
	...
	/* on ajoute une valeur a l'oposé de la direction apuillé*/
	private boolean ajouteval(Deplacement direction) {
		...
		if (direction == Deplacement.GAUCHE){
			if(possible_ajoute(1,this.getTaille_x())){// on regarde si il est possible d'ajouter
				while(!res){ // tant que l'on a pas ajouté de valeur
	...
	//fonction d'affichage temporaire pour valider le modèle
