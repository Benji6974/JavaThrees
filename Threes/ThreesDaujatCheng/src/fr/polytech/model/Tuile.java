...
public class Tuile extends Observable {
	...
	/* on deplace la tuille actuelle dans la grille*/
		...
		for(Tuile t : g.getGrille()){
			...
			if((t.getPos_x() == x) && (t.getPos_y() == y)){
				...
				if (t.valeur == 0 || (t.valeur == 2 && this.valeur == 1) || (this.valeur == 2 && t.valeur == 1 ) || ( t.valeur == this.valeur && (t.valeur != 1 && t.valeur !=2))){
					...
					t.valeur += this.valeur; // on ajoute la valeur de la nouvelle tuile
					this.valeur = 0;
					//System.out.println("coucou");
					setChanged();
					notifyObservers(); // ont notifie la vue
					// normalement ca devrait marcher mais encore en cours de developpement
					return true;
	...
	/* on test si il est possible de modifier la tuille actuelle dans la grille*/
